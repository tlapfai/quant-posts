...
Therefore, we need an initial guess of $\sigma_{1}$ as input together with $K$, and obtain $\Delta_{put}$ then interpolate $\sigma$ from the $\Delta$-$\sigma$ quotation. Usually $\sigma_{ATM}$ is chosen to be the initial guess. The output $\sigma_{2}$ will be different from our input $\sigma_{1}$, so that we use $\sigma_{2}$ as input and repeat the step, until the input $\sigma_{n}$ and ouput $\sigma_{n+1}$ are close enough.
...
In this `TargetFun` class, $K$ (`strike`) is decided in the initialization stage. After that, in the call stage, we can plug the guess `v0` as $\sigma_{i}$ into `TargetFun(v0)` then obtain the difference of $\sigma_{i+1}$ and $\sigma_{i}$.
